...
func TestDeleteUserAppSessions(t *testing.T) {
	...
	for _, user := range users {
		...
		err = identity.UpsertAppSession(context.TODO(), session)
		require.NoError(t, err)
	}

	// Ensure the number of app sessions is correct.
	sessions, err := identity.GetAppSessions(context.TODO())
	require.NoError(t, err)
	require.Len(t, sessions, 2)

	// Delete sessions of the first user.
	err = identity.DeleteUserAppSessions(context.TODO(), &proto.DeleteUserAppSessionsRequest{Username: users[0]})
	require.NoError(t, err)
	...
	err = identity.DeleteUserAppSessions(context.TODO(), &proto.DeleteUserAppSessionsRequest{Username: users[1]})
	require.NoError(t, err)
