...
func main() {
	workflow, token, ripplingToken, reviewers, err := parseFlags()
	if err != nil {
	...
	b, err := createBot(ctx, token, ripplingToken, reviewers)
	if err != nil {
...
func parseFlags() (string, string, string, string, error) {
	...
	return *workflow, *token, *ripplingToken, string(data), nil
}

func createBot(ctx context.Context, token string, ripplingToken string, reviewers string) (*bot.Bot, error) {
	...
	reviewer, err := review.FromString(reviewers, ripplingToken)
	if err != nil {
		return nil, trace.Wrap(err)
	}
